...
class NoteServiceNoteCreateTest {
    ...
    void createNewUserNote_promptTrue_aiSuccess_savesAiTextAndCost() {
        ...
        PromptUtil.ChatResult chatResult =
                new PromptUtil.ChatResult("AI text", 100, 100, 100, 0.50); // cost = 0.50$

        when(openRouterService.chat(
                eq(user.getModel().getValue()),
                eq(user.getPrompt()),
                eq(request.text())
        )).thenReturn(chatResult);

        when(noteRepository.getTotalCostUSDByUserIdByMonth(
                eq(userId),
                anyInt(),
                anyInt())
        ).thenReturn(0.005);
        ...
        verify(openRouterService).chat(
                user.getModel().getValue(),
                user.getPrompt(),
                request.text()
        );
    ...
    void createNewUserNote_promptTrue_aiThrowsGenericException_fallsBackToOriginalText() {
        ...
        when(openRouterService.chat(any(), any(), any()))
    ...
    void createNewUserNote_promptTrue_monthlyLimitExceeded_throwsException() {
        ...
        PromptUtil.ChatResult chatResult =
                new PromptUtil.ChatResult("AI text", 100, 100, 100, 1.0); // cost > 0
